...
LOCALSSD_ZONE=asia-east1-b
BYPASS_PROXY_REGION=asia-east1
CONFIG_BUCKET_NAME=xhs-cdn-config

IMAGE_NAME=image-nginx-cdn-20221027
...
LOCALSSD_INSTANCE_NUM=25
LOCALSSD_PER_INSTANCE=24 #375 GB per disk
...
BYPASS_MIG_INSTANCE_MAX=50

#configuration of LB
LB_NAME=lb-cdn-nginx-proxy
LB_PUBLIC_IP_NAME=public-ip-cdn-lcoal-storage
LB_BACKEND_NAME=("video-xhs-backend" "ads-video-al-backend" "dc-xhscdn-com-backend")
...
if [ $1 = 'create' ]; then
    ...
    gcloud beta compute instance-templates create $LOCALSSD_TEMPLATE_NAME \
        ...
        --metadata=cdn-config-bucket=$CONFIG_BUCKET_NAME,nginx-proxy-type=localssd \
        --maintenance-policy=TERMINATE \
        --provisioning-model=STANDARD \
        --scopes=https://www.googleapis.com/auth/cloud-platform \
        --tags=http-server,https-server \
        --create-disk=auto-delete=yes,boot=yes,device-name=$LOCALSSD_TEMPLATE_NAME,image=projects/$PROJECT_ID/global/images/$IMAGE_NAME,mode=rw,size=50,type=pd-balanced \
    ...
    gcloud compute instance-templates create $BYPASS_TEMPLATE_NAME \
        --machine-type=$MACHINE_TYPE \
        --network-interface=network=default,network-tier=PREMIUM,address='' \
        --metadata=cdn-config-bucket=$CONFIG_BUCKET_NAME,nginx-proxy-type=bypass \
    ...
    gcloud compute url-maps add-path-matcher $LB_NAME --path-matcher-name=${LB_BACKEND_NAME[0]} --default-service=${LB_BACKEND_NAME[0]} --new-hosts='sns-video-qn.xhscdn.com,sns-video-hw.xhscdn.com,sns-video-qc.xhscdn.com,sns-video-bd.xhscdn.com,sns-video-exp1-qc.xhscdn.com,sns-video-exp2-qc.xhscdn.com'
    gcloud compute url-maps add-path-matcher $LB_NAME --path-matcher-name=${LB_BACKEND_NAME[1]} --default-service=${LB_BACKEND_NAME[1]} --new-hosts='ads-video-al.xhscdn.com'
    gcloud compute url-maps add-path-matcher $LB_NAME --path-matcher-name=${LB_BACKEND_NAME[2]} --default-service=${LB_BACKEND_NAME[2]} --new-hosts='dc.xhscdn.com'
